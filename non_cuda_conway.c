 ...
 * This is the non-CUDA version, for comparison
 */
...
// non-CUDA function for determining whether a cell should be dead or alive
void conway(int i, int *conwayBoard, int *newConwayBoard, int numCols, int numRows, int numCells){
    /** From Wikipedia:
...
int main(int argc, char* argv[]){
    ...
    // allocate memory for the boards (on CPU)
    conwayBoard = malloc(sizeof(int) * numCells);
    newConwayBoard = malloc(sizeof(int) * numCells);
    ...
    for(int genNumber = 1; genNumber <= numGenerations; genNumber++){
        ...
        // for loop that figures out whether or not each cell is dead or alive
        for(int i = 0; i < numCells; i++){
            
            conway(i, conwayBoard, newConwayBoard, numCols, numRows, numCells);
            
        }

        // for loop that copies the new conway board into the original conway board
...
//    printf("Converting to GIF...\n");
    ...
    printf("Conway's Game of Life has finished\n");
